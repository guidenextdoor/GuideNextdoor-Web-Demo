...
import { consumeAuthRedirect, getCurrentSession, signUpWithPassword, signInWithPassword } from '../lib/database';

export default function LoginView() {
  const { t, i18n } = useTranslation();
  const navigate = useNavigate();
  const [searchParams] = useSearchParams();
  const redirectPath = getSafeRedirectPath(searchParams.get('redirect'), i18n.language);
  ...
  }, [session, status.saving, navigate, redirectPath]);
    ...
    >
      <div className="grid w-full overflow-hidden rounded-lg border border-gnd-cream bg-white shadow-xl shadow-red-900/5 lg:grid-cols-[minmax(0,0.95fr)_minmax(420px,1fr)]">
        <aside className="hidden border-r border-gnd-cream bg-gnd-cream/25 p-8 lg:flex lg:flex-col">
          <div>
            <p className="text-xs font-black uppercase tracking-[0.22em] text-gnd-red">{t('auth.contextEyebrow')}</p>
            <h2 className="mt-3 text-3xl font-black tracking-tight text-gnd-dark">{t('auth.contextTitle')}</h2>
            <p className="mt-3 max-w-sm text-sm font-bold leading-6 text-gnd-gray">{t('auth.contextSubtitle')}</p>
          </div>
        ...
        <div className="p-6 md:p-8">
          <p className="text-xs font-black uppercase tracking-[0.22em] text-gnd-red">{t('auth.eyebrow')}</p>
          <h1 className="mt-2 text-3xl font-black tracking-tight text-gnd-dark sm:text-4xl">{mode === 'signup' ? t('auth.signupTitle') : t('auth.title')}</h1>
          <p className="mt-3 max-w-xl text-sm font-bold leading-6 text-gnd-gray">{t('auth.subtitle')}</p>
          ...
          {session ? (
            ...
            <div className="mt-6">
              <div className="grid grid-cols-2 rounded-lg bg-gnd-cream p-1">
